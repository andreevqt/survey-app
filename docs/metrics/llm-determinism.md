...
- 9 моделей: по дешёвому и топовому варианту от DeepSeek, Google, OpenAI, Anthropic
  (+ отдельно Claude Opus 4.8).
- Скрипт: `/tmp/determinism.mjs` (вызовы через RouterAI; ключ не коммитится).

### Что такое temperature

`temperature` — параметр генерации, который управляет «случайностью» выбора
следующего токена. Модель на каждом шаге выдаёт распределение вероятностей по
словарю; temperature масштабирует это распределение перед выбором:

- **`temperature = 0`** — жадный выбор: всегда берётся самый вероятный токен.
  Теоретически это должно давать один и тот же ответ на один и тот же вход —
  поэтому мы тестировали именно на нём (самый строгий случай).
- **выше `0`** (например, 0.7–1.0) — распределение «сглаживается», менее вероятные
  токены получают шанс → ответы разнообразнее, но менее предсказуемы.

Важно: даже при `temperature = 0` детерминизм **не гарантирован** — выбор токена
зависит от порядка операций с плавающей точкой на GPU, батчинга запросов на стороне
провайдера и MoE-роутинга (в больших моделях вход направляется в разные «эксперты»).
Наш бэкенд использует `temperature = 0.2` (компромисс: связный, но не «деревянный»
текст), так что в проде разброс будет ещё больше, чем в этом тесте.

### Как считались метрики

На каждый вопрос делается 5 прогонов; всего 3 вопроса = 15 прогонов на модель.

- **byte-identical** — для каждого вопроса берём первый из 5 ответов как эталон и
  считаем, сколько из 5 побитово (посимвольно) ему равны. Суммируем по 3 вопросам →
  «X / 15». Самая строгая метрика: даже лишний пробел или иной порядок ключей в JSON
  ломает совпадение.

- **разброс sentiment** — из каждого ответа берём число `sentiment.positive`.
  Для одного вопроса считаем `max − min` этого числа по 5 прогонам (в процентных
  пунктах) и берём максимум по всем вопросам. Показывает, насколько «гуляет» оценка
  тональности на идентичном входе. 0 п.п. = тональность всегда одинаковая.

- **стабильность тем (Jaccard)** — насколько совпадает *набор* тем между прогонами.
  **Коэффициент Жаккара** для двух множеств A и B — это размер пересечения, делённый
  на размер объединения: `J(A,B) = |A ∩ B| / |A ∪ B|`. Значение от 0 до 1:
  `1` — наборы тем полностью совпали, `0` — ни одной общей темы, `0.5` — половина
  общих. Пример: эталон дал темы `{pricing, support}`, повтор — `{pricing, speed}`:
  пересечение `{pricing}` = 1, объединение `{pricing, support, speed}` = 3, значит
  `J = 1/3 ≈ 0.33`. Мы берём множество меток `themes[].label`, сравниваем каждый из
  прогонов 2–5 с первым и усредняем Jaccard. (Только метки тем; их `count`/`quote`
  не учитываются.)
...
| `anthropic/claude-opus-4.8` | 3 / 15 | 12 п.п. | 0.41 |
...
1. **`temperature = 0` ≠ детерминизм.** Ни одна из 9 моделей не дала 15/15 на
...
2. **Дороже ≠ стабильнее — часто наоборот.** GPT-5.5, Claude Sonnet 4.6 и
   Claude Opus 4.8 — все по 3/15, заметно менее воспроизводимы, чем дешёвые
   4o-mini (8/15) и Haiku (10/15). То есть и самая дорогая модель в тесте (Opus)
   не выигрывает по детерминизму. Вероятно, крупные reasoning/MoE-модели «плывут» сильнее.
